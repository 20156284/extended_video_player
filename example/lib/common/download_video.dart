...
class DownloadUtil {
  ...
  static String hiveBox = 'video_cache';

  static void removeTask({required String delId}) {
  ...
  // 获取地址
  static Future<String> getPath({required String folderName}) async {
    Directory? documents;
  ...
  // 获取地址
  static Future<String> getEnvironmentPath({required String folderName}) async {
  ...
  // 初始化下载状态
  static void initStatus({required int finishNum}) {
    downloading = true;
    currentRemove = false;
  ...
  static Future<void> createDownloadTask({
    ...
    try {
      ...
      // 获取储存地址
      final saveDirectory =
          await getPath(folderName: '${DateTime.now().millisecondsSinceEpoch}');
      // 存储本地m3u8文件
...
// 单个下载方法
  ...
  //获取本地唯一标识
  static Future<String> getUniqueId({
    Function? decry,
    Function? encode,
    String? folderName,
  }) async {
    final saveDirectory =
        await getEnvironmentPath(folderName: folderName ?? ''); // 获取储存地址
    ...
    try {
      ...
      var units = base64Decode(cx);
      if (decry != null) {
        units = base64Decode(decry(cx));
      }

      final txt = String.fromCharCodes(units);
  ...
  static Future<void> setUniqueId({
    ...
    try {
      ...
      var txt = base64Encode(bytes);
      if (encode != null) {
        txt = encode(base64Encode(bytes));
      }

      json['uni'] = txt;
      final path = await File('${saveDirectory}unis.json')
